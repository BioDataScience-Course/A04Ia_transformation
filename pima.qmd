...
<!--% Importez les données Pima.tr2 et Pima.te provenant toutes les deux du package {MASS}. Les deux tableaux sont combinés à l'aide de la fonction `sbind_rows()`. -->
...
pima <- rename_(pima, n_pregnant = 'npreg', glucose = 'glu', diastolic = 'bp',
...
[] -   Il y a plusieurs valeurs manquantes dans ce jeu de données. La variable qui contient le plus grand nombre de valeurs manquantes est la variable `triceps`.

[] -  La variable `n_pregnant` est reconnue dans R comme une variable **integer**. Cette variable doit être convertie en **double**.
[] -  La variable `n_pregnant` est reconnue dans R comme une variable **integer**. Le nombre d'enfants ne peut prendre que des valeurs entières.}-")
```
