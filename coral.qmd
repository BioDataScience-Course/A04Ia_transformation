...
<!--% Complétez les instructions ci-dessous pour importer les données depuis le lien fourni dans le README.md. -->
...
[] - La variable `salinity` ne comprend qu’une seule valeur, alors que la variable `temperature` en comprend plusieurs.
...
<!-- Réalisez un tableau résumé du nombre d'individus par espèce et par aquarium. Ce tableau doit comprendre les colonnes `species`, `localisation` et `n`. Assignez votre résultat à `coral_count`. Formatez votre résultat avec la fonction tabularise(). -->

La répartition des espèces par aquariums est présentée dans le tableau ci-dessous.

```{r ccount, record='RODFS', object='coral_count', warning=FALSE}
coral %>.%
  ___ -> coral_count
___(coral_count)
```
...
```{r ctab, record='RODFS', object='coral_tab', warning=FALSE}
coral %>.%
  ___ -> 
  coral_tab
___
